...
with revit.Transaction("Filters by Value", doc):
    for param_value in values:
        ...
        prefix = "Category_parameter_rule_"
        suffix = "_kw"
        # filter_name = param_dict[selected_parameter].replace(SHARED_PARAMETER_LABEL, "") + " - " + value_name #code goc
        filter_name = prefix + value_name + suffix#Code theo tieu chuan kw
